...
It needs very little resources. Its event-driven architecture allows it to 
easily handle thousands of simultaneous connections on hundreds of instances
...
%build
%{__make} -j8 CFLAGS="__cflags__" LDFLAGS="${CFLAGS} -static" USE_CPU_AFFINITY=1 USE_THREAD=1 USE_ZLIB=1 USE_OPENSSL=1 USE_PCRE2=1 USE_PCRE2_JIT=1 PCRE2DIR=/opt ZLIB_LIB=/opt/lib ZLIB_INC=/opt/include USE_FUTEX=1 USE_PTHREAD_SHARED=1 DEBUG="-DDEBUG_THREAD" SSL_INC=/opt//include SSL_LIB=/opt/lib ARCH=%{_target_cpu} CPU=__cpu__ TARGET=linux2628 ADDLIB=-lpthread
...
%changelog
* Sun Jan 13 2019 Nils Meyer <nils@nm.cx>
- updated to __version__
- adjusted to use lto, templated, static linking
